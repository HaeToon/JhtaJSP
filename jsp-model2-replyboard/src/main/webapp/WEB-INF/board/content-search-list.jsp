...
<div class="container">
    ...
    <nav aria-label="Page navigation example" class="mt-5 mb-5">
        <ul class="pagination d-flex justify-content-center">
            <li class="page-item">
                <a class="page-link" href="../board/content-search?page=1" aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
            <c:choose>
                <c:when test="${param.page<=paginationPerPage}">
                    <li class="page-item disabled"><a class="page-link" href="../board/content-search?page=${paginationStart-1}&search=${search}&searchWord=${searchWord}">이전</a></li>
                </c:when>
                <c:otherwise>
                    <li class="page-item"><a class="page-link" href="../board/content-search?page=${paginationStart-1}&search=${search}&searchWord=${searchWord}">이전</a></li>
                </c:otherwise>
            </c:choose>
            <c:forEach begin="${paginationStart}" end="${paginationEnd}" step="1" var="page">
                <c:choose>
                    ...
                    <c:otherwise>
                        <li class="page-item "><a class="page-link"href="../board/content-search?page=${page}&search=${search}&searchWord=${searchWord}">${page}</a></li>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            <c:choose>
                <c:when test="${param.page/5>=totalPagination-1}">
                    <li class="page-item disabled"><a class="page-link" href="../board/content-search?page=${paginationStart+paginationPerPage}&search=${search}&searchWord=${searchWord}">다음</a>
                    </li>
                </c:when>
                <c:otherwise>
                    <li class="page-item"><a class="page-link" href="../board/content-search?page=${paginationStart+paginationPerPage}&search=${search}&searchWord=${searchWord}">다음</a>
                    </li>
                </c:otherwise>
            </c:choose>
            <li class="page-item">
                <a class="page-link" href="../board/content-search?page=${totalPage}&search=${search}&searchWord=${searchWord}" aria-label="Next">
